...
class ShuttleDumperShellCommand extends ShuttleDumper
{
    function dump($export_file_location, $table_prefix = '')
    {
        ...
        if (ShuttleDumpFile::is_gzip($export_file_location)) {
            $command .= ' | gzip';
        }
        ...
        if ($return_val !== 0) {
            ...
            unlink($error_file);
            throw new ShuttleException('Couldn\'t export database: ' . $error_text);
        }
